...
const JobDetail = ({ jobs }) => {
    ...
    return (<div className="max-w-xl mx-auto p-4 sm:p-6 bg-white shadow rounded-md">
        <h2 className="text-xl sm:text-2xl font-bold mb-2">{job.position}</h2>
        <p className="text-sm sm:text-base text-gray-600 mb-1">{job.company}</p>
        ...
        <div className="mb-2">
            <span className="block text-sm sm:text-base font-medium text-gray-700">Applied On:</span>
            <span className="text-sm text-gray-800">{job.appliedDate || '—'}</span>
        </div>

        <div className="mb-4">
            <span className="block text-sm sm:text-base font-medium text-gray-700">Notes:</span>
        ...
        <div className="flex flex-col sm:flex-row justify-between gap-2 mt-6">
            <button
                onClick={() => navigate(-1)}
                className="w-full sm:w-auto text-sm text-gray-600 hover:underline"
            >
                ← Back
            </button>
            <Link
